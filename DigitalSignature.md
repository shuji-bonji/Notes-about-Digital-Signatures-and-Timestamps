...
1. **認証（Authentication）**: 送信者が本人であることを確認できること。

2. **完全性の保証（Integrity）**: データや文書が変更されていないこと。

3. **非改ざん性/否認防止 （Non-repudiation）**: 送信者がメッセージを否認できないこと。
...
|鍵の種類|利用者|役割|目的|利用方法|
|---|---|---|---|---|
|秘密鍵|(秘密鍵を所有する)署名する本人|署名生成|デジタル署名を通じてデータの整合性と認証を提供|データのメッセージダイジェスト（ハッシュ値）に対して秘密鍵を使用して暗号化し署名を生成し、この署名を公開鍵を持つ人なら誰でも検証できるようにします。これにより、データが送信者によって署名され、その後改ざんされていないことが証明されます。|
|公開鍵|公開鍵を知っている誰でも|署名を検証|データの整合性と送信者の認証を確認|デジタル署名を公開鍵を使って復号し得られたデータのメッセージダイジェスト（ハッシュ値）と、同じハッシュ関数を用いて取得したデータのメッセージダイジェスト（ハッシュ値）算出し、署名内のハッシュ値と算出したハッシュ値とを比較して検証します。これにより、データが改ざんされずに送信されたことと、正当な送信者から来たことが確認されます|
...
1. ハッシュ生成:  
**送信者（アリス）** は、送信するメッセージからハッシュ値を生成するために **ハッシュ関数** を使用します。このハッシュ値は、メッセージの一意的なデジタル指紋として機能し、メッセージの整合性を保証するのに役立ちます。
...
## デジタル署名の用途
デジタル署名は、主に次のような用途で使用されます。

1. **データの完全性保証**:  
デジタル署名は、データが送信後に改ざんされていないことを保証します。受信者はデジタル署名を検証することで、データが変更されていないことを確認できます。

2. **送信者の認証**:  
送信者が自分の秘密鍵でデータに署名することにより、受信者は公開鍵を使用してその署名を検証し、送信者の身元を確認できます。

3. **否認防止**:  
送信者は自分の秘密鍵で署名したことにより、後でその行為を否認することができません。これにより、トランザクションの法的な拘束力を確保することができます。

4. **ソフトウェア配布**:  
ソフトウェア開発者は、配布するソフトウェアのデジタル署名を行い、エンドユーザーが正規のソフトウェアをダウンロードしていることを保証します。これにより、マルウェアのリスクを軽減します。
...
過去に署名された PDF ファイルのデジタル署名について、その有効期限が過ぎた後も検証可能かどうかは、PDF がどのように署名されているかで違ってきます。
