...
graph TB
    subgraph bob["ボブ"]
        ...
        A_ciphertext_in_B["アリスからの暗号文"]　--プライベート鍵を利用--> B_priKey
        B_priKey --復号化--> A_msg_in_B["メッセージ"]
    end

    subgraph auth["認証局"]
        subgraph Auth_pairKey["ペア鍵"]
            Auth_prikey["認証局のプライベート鍵"]
            Auth_pubkey["認証局の公開鍵"]
        end
        subgraph B_cert["証明書"]
...
1. **ボブ**:
   ...
   - **鍵ペア生成(公開鍵 & 秘密鍵)**: ボブは自分の鍵ペアを生成します。
   - **秘密鍵で複合**: ボブは受け取った暗号文を自分の秘密鍵で複合します。
...
graph TB
    subgraph alice["アリス"]
        ...
        A_hash --> A_priKey
        A_priKey --プライベート鍵を用いてハッシュ値を暗号化--> A_sign["デジタル署名"]
    end
    ...
    subgraph auth["認証局"]
        ...
        Auth_prikey --認証局のプライベート鍵で<br>デジタル署名--> Auth_sign
    end
...
1. **アリス**:
   ...
   - **鍵ペア生成(公開鍵 & 秘密鍵)**: アリスは自分の鍵ペアを生成します。
   - **公開鍵の登録**: 認証局に公開鍵を登録します。
   - **メッセージのハッシュ値を算出**: メッセージのハッシュ値を算出します。
   - **メッセージにデジタル署名**: ハッシュ値をプライベート鍵で暗号化しデジタル署名を行い、メッセージに添付します。
